...
def processFunction(cursor):
    ...
    while row is not None:
        ...
        '''
        if count % 10000 == 0:
            print("Putting information into the DB.")
            doInserts(cacheDict, cursor)
            cacheDict = {}
        '''
...
def doInserts(cacheDict, cursor):
    print("inserting data, something is not wrong in whoville")
    for key in cacheDict:
        ...
        region = splits[2]
        if len(champs) == 1:
            insertor(cursor, "singlecache", " (ChampOne, Patch, Region, Games, Wins) ", (champs[0], patch, region, cacheDict[key][1], cacheDict[key][0]))
        elif len(champs) == 2:
            insertor(cursor, "doublecache", " (ChampOne, ChampTwo, Patch, Region, Games, Wins) ", (champs[0], champs[1], patch, region, cacheDict[key][1], cacheDict[key][0]))
        elif len(champs) == 3:
            insertor(cursor, "triplecache", " (ChampOne, ChampTwo, ChampThree, Patch, Region, Games, Wins) ", (champs[0], champs[1], champs[2], patch, region, cacheDict[key][1], cacheDict[key][0]))
        elif len(champs) == 4:
            insertor(cursor, "quadruplecache", " (ChampOne, ChampTwo, ChampThree, ChampFour, Patch, Region, Games, Wins) ", (champs[0], champs[1], champs[2], champs[3], patch, region, cacheDict[key][1], cacheDict[key][0]))
        elif len(champs) == 5:
            insertor(cursor, "quintuplecache", " (ChampOne, ChampTwo, ChampThree, ChampFour, ChampFive, Patch, Region, Games, Wins) ", (champs[0], champs[1], champs[2], champs[3], champs[4], patch, region, cacheDict[key][1], cacheDict[key][0]))
